...
export default function NewReservationScreen() {
  ...
  const handleGuestChange = (key: keyof ReservationFormData, value: number) => {
    ...
    if (selectedProperty) {
      ...
      setForm((prev) => ({ ...prev, [key]: value, ...suggestion }));
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    if (form.nb_couples + form.nb_solo_adults + form.nb_children + form.nb_babies === 0) { setError('Au moins 1 voyageur requis'); return; }
  ...
  return (
    <SafeAreaView style={styles.safeArea} edges={['top']}>
      ...
      >
        ...
        <Text style={styles.title}>{t('reservations.new')}</Text>
      ...
      <ScrollView style={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.chipRow}>
          {(properties ?? []).map((p) => (
            <TouchableOpacity
              key={p.id}
              style={[styles.propertyChip, { borderColor: p.color }, selectedProperty?.id === p.id && { backgroundColor: p.color }]}
              onPress={() => handlePropertySelect(p)}
            >
              <Text style={[styles.propertyChipText, { color: selectedProperty?.id === p.id ? '#FFFFFF' : p.color }]} numberOfLines={1}>
                {p.name}
              </Text>
            </TouchableOpacity>
          ))}
        ...
        <View style={[styles.section, SHADOWS.sm]}>
          <TouchableOpacity style={styles.datePicker} onPress={() => openDatePicker('check_in')}>
            ...
            <Text style={[styles.datePickerValue, !form.check_in && styles.datePlaceholder]}>{form.check_in || 'Sélectionner…'}</Text>
          </TouchableOpacity>
          <TouchableOpacity style={styles.datePicker} onPress={() => openDatePicker('check_out')}>
            ...
            <Text style={[styles.datePickerValue, !form.check_out && styles.datePlaceholder]}>{form.check_out || 'Sélectionner…'}</Text>
          </TouchableOpacity>
      ...
      <Snackbar visible={overlapWarning} onDismiss={() => setOverlapWarning(false)} duration={4000} style={{ backgroundColor: APP_COLORS.danger }}>
...
const styles = StyleSheet.create({
  ...
  datePlaceholder: { color: APP_COLORS.textTertiary, fontWeight: '400' },
